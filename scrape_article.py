...
Log('Apple Daily: scrape_article starts...')

# Program begins
if len(sys.argv)<2:
    print('Pls. pass the <article URL> as parameter.')
    print('Usage: python scrape_article.py <article URL>')
    quit()  # Exit program
...
domain = '' # Already has domain name for Apple Daily
...
#r = Render(oncc_article_url)
Log('Saving web page using phantomjs...')
cmd = 'phantomjs '+project_path+'/saveWebPage.js '+oncc_article_url+' > '+articleTmpFile
os.system(cmd)
...
for aTag in aTags:
    if aTag.has_attr('title') and aTag.has_attr('class'):
        if aTag['class'][0] == 'fancybox-button':
#            print(aTag['title'])
#            print(aTag['href'])
...
output = template.render(heading=heading, output=output_list, oncc_article_url=oncc_article_url)
#print(output)
#filename = 'oncc_' + time.strftime("%Y%m%d") + '.html'
filename = os.path.join(folder, articleId) + '.html'  # Need to add file extension for Apple Daily
